// LA PRESENTE TABLA ES DE MIS PACIENTES PRIVADOS 
...
function listar() {
    ...
    if(tablaPaciente.length > 0){
        for(const i in tablaPaciente){
            ...
            dataFila += "<td>"+
                        "<button type='button' class='btn btn-warning' onclick='abrirForm("+varPaciente.idPaciente+")'>VERIFICAR</button>"+
                        "<button type='button' class='btn btn-info' onclick='eliminarItem("+varPaciente.idPaciente+")'>QUITAR</button>"+
                        "</td>";
            dataFila += "</tr>";
